...
describe('timeAgo', () => {
  ...
  it('returns "1 min ago" for exactly 1 minute', () => {
    expect(timeAgo(dateMinutesAgo(1))).toBe('1 min ago');
  });

  it('returns "X mins ago" for 2-59 minutes', () => {
    expect(timeAgo(dateMinutesAgo(5))).toBe('5 mins ago');
    expect(timeAgo(dateMinutesAgo(30))).toBe('30 mins ago');
    expect(timeAgo(dateMinutesAgo(59))).toBe('59 mins ago');
  });

  it('returns "1 hour ago" for exactly 60 minutes', () => {
    expect(timeAgo(dateMinutesAgo(60))).toBe('1 hour ago');
  });

  it('returns "X hours ago" for 2-23 hours', () => {
    expect(timeAgo(dateMinutesAgo(120))).toBe('2 hours ago');
    expect(timeAgo(dateMinutesAgo(720))).toBe('12 hours ago');
  });
  ...
  it('returns "X days ago" for 2-6 days', () => {
    expect(timeAgo(dateMinutesAgo(60 * 48))).toBe('2 days ago');
    expect(timeAgo(dateMinutesAgo(60 * 24 * 6))).toBe('6 days ago');
  });
  ...
  it('handles ISO string input', () => {
    ...
    expect(timeAgo(iso)).toBe('5 mins ago');
  });
